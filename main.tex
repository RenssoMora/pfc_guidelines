...
\title{\LARGE\bfseries\color{tituloColor} Guia general y recomendaciones para la redaccion de una tesis  del área de Computacion}
\author{Departamento de Computacion-UTEC\\}
\date{\today}
...
\begin{abstract}
\noindent
Este documento presenta un \emph{molde argumentativo} que
explica que debe \textbf{existir} dentro de una tesis de Ciencia de la
Computacion para que esta sea correcta, autocontenida y defendible, con
independencia de los nombres que finalmente reciban sus capitulos. Sobre 
esa base se identifican cuatro funciones que todo trabajo de investigacion 
en computacion debe satisfacer: situar el problema, sostenerse a si mismo con la teoria
necesaria, distinguirse de lo ya hecho, y ofrecer una contribucion con
evidencia, análisis y discusión, para finalemente terminar con la sintesis del
estudio realizado. El objetivo no es imponer una estructura rigida sino asegurar que, 
sin importar como se titulen las secciones, la \emph{esencia} de cada funcion este presente.
\end{abstract}
...
cumple un rol argumentativo especifico. \citet{zobel2014writing} declara que escribir en computacion es, ante todo, \emph{construir un argumento
...
negociable. En general el documento de tesis tiene las siguientes caracteristicas: i) explicativa, 
ii) autocontenida, iii) replicable, iv) verificable. Para garantizar que cumpla con estas caracteristicas
es necesario que cierto contenido, la escencia de ellos, se encuentre dentro del documento, sin importar 
el nombre que se le de a cada seccion. Ahora bien, dada el área de estudios, existen ciertas
libertades que se dan en cuanto a la ubicación donde se encuentra ese contenido, sin embargo, 
hay una estructura lógica que se debe seguir tomando en cuenta que el documento de tesis
tiene forma de ensayo discurso. Por ende como todo discurso tiene introducción, desarrollo y conlusión.
La tesis también debe seguir esa estructura. No obstante, la disposición de la información
puede variar según el tipo de contribución que se haga, el área de investigación y el tipo de tesis 
(pregrado, maestría, doctorado), y finalmente el propio estilo del autor. 
...
Aquí es necesario resaltar que el problema rerpresenta desafios, por ejemplo
en el caso de un algoritmo es aquello que le impide llegar a cumplir su objetivo.

\begin{cajaAlerta}
Un error comun es confundir el problema computacional con el objetivo de la tesis: 
``el problema'' no es el algoritmo que se propone, ni el teorema que se prueba, etc.
\end{cajaAlerta}

\subsection{Objetivos, preguntas de investigacion e hipotesis}

El objetivo de una tesis es la promesa que se hace al lector. Dependiendo
del área de investigación ese objetivo se relaciona con un algoritmo, 
un teorema, un modelo, una solución, etc. 
...
Note que dependiendo del área y del matiz del trabajo, en algunas tesis se 
presenta de manera formal las hipótesis, al igual que las preguntas de investigación, mientras que en otras se
o algunas veces se encuentran de manera más informal dentro de los párrafos de la introducción,
lo que si nunca puede faltar y quedar resaltado son los objetivos de la investigación.
...
En la Figura~\ref{fig:intro} se muestra un ejemplo de como se puede organizar 
la introducción de una tesis, note que el formato usualmente se ve como si fuera
una especie de embudo que va desde lo general a lo particular, es decir, 
desde el contexto hasta los objetivos de la investigación (estrategia Top Down).
Una particularidad de este gráfico es que permite separar la problemática del 
problema computacional. Esto es conveniente porque permite al lector entender 
el contexto y la problemática que se quiere resolver para luego comprender los desafíos
que la contribución propuesto debe superar para cumplir con los objetivos 
de la investigación. 

\begin{figure}[h]
\centering
\includegraphics[width=0.99\linewidth]{intro.pdf}
\caption{Formato sugerido para la introduccion de una tesis. 
Los nombres de seccion son intercambiables.}
\label{fig:intro}
\end{figure}
...
limitaciones concretas de soluciones concretas. Una sugerencia es: 
...
conectarlos con los objetivos de la tesis. La justificación de la contribución
respecto a lo que ya existe es la parte mas importante de este bloque; y no tiene formato
de monografía cronológica, sino de comparativa critica. 
\end{cajaAlerta}
...
\cabeceraTabla{Tipo de contribucion} & \cabeceraTabla{Evidencia minima obligatoria} & \cabeceraTabla{Evidencia adicional recomendada}\\
\midrule
...
\begin{enumerate}
  ...
  alcance. \textit{(Bloque a)}
  \item \textbf{Fundamentos teoricos}, definiciones, notacion, resultados
  previos usados como base. \textit{(Bloque b, parte teorica)}
  \item \textbf{Estado del arte}, trabajos relacionados y brecha
  identificada. \textit{(Bloque b, parte comparativa)}
  \item \textbf{Contribucion propuesta}, algoritmo, modelo, teorema o
  sistema, con su justificacion formal. \textit{(Bloque c, parte
  formal)}
  \item \textbf{Diseño experimental y resultados}, \emph{solo si aplica
  segun la \cref{tab:resumen}}. \textit{(Bloque c, parte empirica)}
  \item \textbf{Conclusiones}, sintesis, limitaciones, trabajo futuro.
  \textit{(Bloque d)}

\end{enumerate}

Aunque no aparece de forma explícita en la lista, es recomendable incluir 
partes importantes como abstract o resumen, y bibliografía al final de la tesis. 
En el caso del abstract, este debe ser un resumen de la tesis, que contenga el problema,
los objetivos, la contribución y los resultados de la investigación. Y evidentemente, 
la bibliografía debe contener todas las referencias citadas en la tesis. Aunque 
puede parecer obvio, en algunos casos es necesario recordarlo.


También es importante resaltar que la estructura de la tesis puede variar 
dependiendo del tipo de investigación, el área de estudio y el nivel académico 
(pregrado, maestría, doctorado). Por lo tanto, es importante que el autor adapte 
esta guía a las necesidades específicas de su trabajo y siga las directrices 
establecidas por su institución académica. Por ejemplo en algunas tesis de maestría, 
el bloque de fundamentos y estado del arte puede fusionarse en un solo capítulo,
mientras que en tesis de doctorado, estos bloques pueden ser más extensos y detallados, 
inclusive en algunas tesis de doctorado, aparecen nuevos capítulos donde cada uno de ellos
presenta un artículo publicado por el autor, o alguna estrategia detallada, 
lo que hace que la tesis sea más extensa y detallada. En cualquier situación, sea cual 
sea, el documento de tesis debe cumplir con las funciones esenciales de situar el problema,
sostenerse a sí mismo con la teoría necesaria, distinguirse de lo ya hecho y
ofrecer una contribución con evidencia de que funciona, cerrando con la síntesis final. 

Note que muchas secciones, capítulos, no han sido detalladas en esta guía, 
ya que la intención es que el autor de la tesis pueda adaptar la estructura de la tesis a 
su propio estilo y necesidades. Por ejemploagradecimientos, apéndices, anexos, etc., son
secciones que pueden ser incluidas en la tesis según el criterio del autor y las normas de 
la institución académica. 

Así mismo, ningún documento de tesis puede ser presentado sin el visto bueno del 
asesor de tesis, quien es el encargado de guiar al estudiante en la redacción de su 
tesis, asegurando que cumpla con los estándares académicos y científicos requeridos por la institución.  
